...
class _MyHomePageState extends State<MyHomePage> {
  Map response = {};
  String contractId = 'chichvirus-contract.mibi.testnet';
  String mutateMethod = 'submitMessage';
  String viewMethod = 'get_matches';

  late Account connectedAccount;

  String signerId = "mibi.testnet";
  String userAccount = "mibi.testnet";
  ...
  void initState() {
    super.initState();
    _textUserIdController.text = "mibi.testnet";
  }
            ...
            children: [
              ElevatedButton(
                ...
                onPressed: () {
                  ...
                  String nearSignInSuccessUrl = 'http://localhost:64056/#/';
                  String nearSignInFailUrl = 'http://localhost:64056/#/';
                  print("OK");
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  response = await NEARTester.callMethodLimitedAccess(contract, method, methodArgs);
                  setState(() {});
                },
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  response = await NEARTester.callMethodLimitedAccessWithDeposit(contract, method, walletURL,
                      methodArgs, 1.0, nearSignInSuccessUrl, nearSignInFailUrl, walletApproveTransactionUrl);
                  setState(() {});
                },
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  String methodArgs = '{"match_id": "match_1"}';
    ...
    if (response.isNotEmpty &&
      ...
      return Card(
        ...
        child: buildCopyableText(
            "Function Response", (utf8.decode(base64.decoder.convert(response['result']['status']['SuccessValue'])))),
      ));
    } else if (response.isNotEmpty && response.containsKey('result') && response['result'].containsKey('result')) {
      String resultDecoded = utf8.decode(response['result']['result'].cast<int>());
      return Card(
          child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 5),
      ...
      child: Padding(
          ...
          child: Column(
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  response = await NEARTester.transferNear(connectedAccount, 1.0, userAccount);
                  setState(() {});
                },
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  response = await NEARTester.callMethodFullAccess(contract, method, methodArgs);
                  setState(() {});
                },
                child: const Text("Call without deposit"),
              ),
              ElevatedButton(
                onPressed: () async {
                  ...
                  response = await NEARTester.callMethodFullAccessWithDeposit(contract, method, methodArgs, 1.0);
                  setState(() {});
                },
                child: Text("Call with ${"1".toString()} Near deposit"),
              ),
...
class NEARTester {
  ...
  static loginWithFullAccess(walletURL, contractId, accountId, appTitle, signInSuccessUrl, signInFailureUrl) {
    // Generate Keys
    var keyPair = KeyStore.newKeyPair();
    ...
    wallet.connectWithFullAccess(contractId, appTitle, signInSuccessUrl, signInFailureUrl, publicKey);
  ...
  static callMethodLimitedAccessWithDeposit(
      Contract contract, String method, String walletURL, args, nearAmount, successUrl, failureUrl, approvalURL) async {
    // Open near wallet in default browser
    var wallet = Wallet(walletURL);

    var result =
        await contract.callFunctionWithDeposit(method, args, wallet, nearAmount, successUrl, failureUrl, approvalURL);
    return result;
  }
