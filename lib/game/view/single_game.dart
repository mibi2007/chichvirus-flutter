...
import 'package:chichvirus/game/app/single_game/game_controller.dart';
...
class ChichVirusSingleGame extends FlameGame with HasTappables, HasDraggables {
  static final paint = Paint()
    ..color = Colors.white
    ..style = PaintingStyle.fill;

  late Sprite startButtonAsset;
  final SingleGameStatsBloc statsBloc;
  final WidgetRef ref;

  // @override
  // bool debugMode = true;

  ChichVirusSingleGame({required this.ref, required this.statsBloc});
  ...
  Future<void> onLoad() async {
    ...
    await add(
      FlameMultiBlocProvider(
        providers: [
          FlameBlocProvider<SingleGameStatsBloc, SingleGameStatsState>.value(
            value: statsBloc,
          ),
        ],
        children: [
          Board(),
          SingleGameStatsController(position: Vector2(-75, 400), levelChangeCallback: renderPieces),
        ],
      ),
    );
  ...
  void checkSolution() async {
    ...
    if (solution.length == 6) {
      // final isCorrect = await game_service.checkSolution((statsBloc.state.level - 1).toString(), solution);
      statsBloc.add(GameEventCheckSolution(solution: solution));
    }
  }
}
